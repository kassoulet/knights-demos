    ...
    if( ddHwDesc.dwFlags )
		{
			...
			MaxWidth = ddHwDesc.dwMaxTextureWidth;
			MaxHeight = ddHwDesc.dwMaxTextureHeight;
		}
    else
		{
	    dwDeviceCaps = ddSwDesc.dpcTriCaps.dwTextureCaps;
			MaxWidth = ddSwDesc.dwMaxTextureWidth;
			MaxHeight = ddSwDesc.dwMaxTextureHeight;
		}
...
//		fprintf(f,"Creating Texture from %s. %ix%ix%i (device max = %ix%i)\n",ptcTexture->strName,ddsd.dwWidth,ddsd.dwHeight,bm.bmBitsPixel,MaxWidth,MaxHeight);
...
//	fprintf(f,"After POW2&SQUAREONLY : %ix%i\n",ddsd.dwWidth,ddsd.dwHeight);
	
//	MaxWidth = 128;
//	MaxHeight = 128;

	if (ddsd.dwWidth>MaxWidth)
	{
		float ratio = (float)ddsd.dwWidth/(float)ddsd.dwHeight;
//		fprintf(f,"Width Error: %ix%i should be:%i ratio=%f\n",ddsd.dwWidth,ddsd.dwHeight,MaxWidth,ratio);
		ddsd.dwWidth = MaxWidth;
		ddsd.dwHeight = ddsd.dwWidth/ratio;
	}
	if (ddsd.dwHeight>MaxHeight)
	{
		float ratio = (float)ddsd.dwWidth/(float)ddsd.dwHeight;
//		fprintf(f,"Height Error: %ix%i sould be:%i ratio=%f\n",ddsd.dwWidth,ddsd.dwHeight,MaxHeight,ratio);
		ddsd.dwHeight = MaxHeight;
		ddsd.dwWidth = ratio*ddsd.dwHeight;
	}

//	fprintf(f,"Final Size : %ix%i\n\n",ddsd.dwWidth,ddsd.dwHeight);
//	fclose(f);
    ...
    if( FAILED( hr = pDD->CreateSurface( &ddsd, &ptcTexture->pddsSurface, NULL ) ) )
        return E_FAIL;
/*
	DDCOLORKEY key={0xffffff};
ptcTexture->pddsSurface->SetColorKey(DDCKEY_DESTBLT+DDCKEY_SRCBLT ,&key);
*/
    // Create the texture
