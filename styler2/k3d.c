...
//  if(y1 < 0) y1 = 0;
...

    {
        ...
        {
            // one Hline
            hline(bmp, fixtoi(px1-1), y, fixtoi(px2+1), color);
            px1 += dx1;
            px2 += dx2;
        }
...
int K3DLoadObjectASC(char * file, struct K3DTObject * obj)
{
    ...
    /* internal helper : load a file line */
    int LoadLine(PACKFILE * f, char * line)
    {
        char c;
        int i = 0;
...
void K3DRotateObject(struct K3DTObject *obj)
{
    ...
    if (obj->rendering.colorModel == Z)
    {
        for (i = 0;i < obj->faces;i++)
        {
            obj->face[i].color = 255-(obj->face[i].z>>16)/obj->zmax;
            //16
...
void K3DBackFaceObject(struct K3DTObject *obj)
{
    ...
    for (i = 0;i < obj->faces;i++)
    {
        if (fixtoi(polygon_z_normal(
                       (V3D*)&obj->rotated[obj->face[i].v1],
                       (V3D*)&obj->rotated[obj->face[i].v2],
                       (V3D*)&obj->rotated[obj->face[i].v3])) > 0)
            obj->face[i].visible = 0 ;
    }
}
