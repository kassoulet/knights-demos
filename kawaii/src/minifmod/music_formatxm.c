...
void FMUSIC_XM_Vibrato(FMUSIC_CHANNEL * cptr)
{
  ...
  {
/*  case 0:
      delta = FMUSIC_SineTable[temp];      // sine
*/
    case 0:
...
//              case 0: cptr->voldelta = FMUSIC_SineTable[temp];                        // sine
    case 0:
      cptr->voldelta = (int) (fabs((sin((float) (cptr->vibpos) * 2 * 3.141592 / 64.0f)) * 256.0f));
      break;
    case 1:
      temp <<= 3;		// ramp down
...
//  FSOUND_Software_SetPan(&FSOUND_Channel[channel], finalpan);
  ...
  if (cptr->notectrl & FMUSIC_FREQ)
  {
    ...
    asm(
      ...
      "mov	%%eax, %0   \n"
      "divl	%%ebx       \n"	// now divide remainder by rate to get lower 32bit fraction
      "mov	%%eax, %1   \n"
      : "=m"(ccptr->speedhi), "=m"(ccptr->speedlo)
      : "a"(freq), "b"(FSOUND_MixRate), "d"(0));
#endif
#ifdef _MSC_VER
    __asm
    {
...
#endif
      // kass
      case FMUSIC_XM_Z_SYNCHRO:
      {
	mod->synchro = current->eparam;
	break;
      }
//                      default:
//                              __assume(0);
...
//                              __assume(0);
	...
	if (current->eparam > 0)
	{
	  switch (mod->tick % 3)
	  {
		...
		cptr->freqdelta = FMUSIC_XM_GetAmigaPeriod(cptr->realnote + paramx, sptr->finetune) - FMUSIC_XM_GetAmigaPeriod(cptr->realnote, sptr->finetune);
#endif
	      break;
	    case 2:
	      if (mod->flags & FMUSIC_XMFLAGS_LINEARFREQUENCY)
		...
		cptr->freqdelta = FMUSIC_XM_GetAmigaPeriod(cptr->realnote + paramy, sptr->finetune) - FMUSIC_XM_GetAmigaPeriod(cptr->realnote, sptr->finetune);
#endif
	      break;
	      //default:
	      //__assume(0);
...
int FMUSIC_LoadXM(FMUSIC_MODULE * mod, FSOUND_FILE_HANDLE * fp)
{
